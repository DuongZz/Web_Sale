...
const STAFF_COLLECTION_SCHEMA = Joi.object({
  username: Joi.string().required().min(8).max(20).trim().strict().external(async (username)=> {
    const isUse = await checkUnique(STAFF_COLLECTION_NAME, "username", username)
    if (isUse) {
  ...
  staffCode: Joi.string().required().min(5).max(10).trim().strict().external(async (staffCode)=> {
    const isUse = await checkUnique(STAFF_COLLECTION_NAME, "staffCode", staffCode)
    if (isUse) {
